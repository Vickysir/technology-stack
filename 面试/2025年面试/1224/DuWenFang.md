...
1. **动态规划思路（O(n²)）**：
   ...
   - 状态转移：对于每个位置 `i`，遍历前面所有位置 `j`（`j < i`）
     - 如果 `nums[j] < nums[i]`，说明可以将 `nums[i]` 接在 `nums[j]` 后面
     - 更新：`dp[i] = max(dp[i], dp[j] + 1)`
   - 初始值：每个位置至少可以形成长度为 1 的子序列，所以 `dp[i] = 1`
   - 最终答案：`max(dp[0...n-1])`

2. **优化思路（O(n log n)）**：

   - 关键观察：对于相同长度的递增子序列，我们只关心末尾元素最小的那个
   - 维护一个 `tails` 数组：`tails[i]` 表示长度为 `i+1` 的递增子序列的最小末尾元素
   - 遍历数组时，对于每个元素 `num`：
     - 如果 `num` 大于 `tails` 中所有元素，说明可以形成更长的子序列，直接追加
     - 否则，用二分查找找到第一个 `>= num` 的位置，替换该位置的元素
   - 最终 `tails.length` 就是答案

3. **思考要点**：
   - 为什么可以用二分查找？因为 `tails` 数组是严格递增的
   - 为什么替换而不是插入？因为我们只关心长度，不关心具体序列
   - 如何理解 `tails` 数组的含义？它维护的是"贪心"策略：让每个长度的子序列末尾尽可能小
