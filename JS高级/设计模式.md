...
我们可以看到，在上面的例子中我们将各个计算奖金算法封装成`一个个策略类`，Bonus的`所有请求`都最终`被委托给具体的策略类`去处理，可以看到通过策略模式重构之后，代码变得更加清晰，各个类的职责更加鲜明。但实际上我们知道上面的类似也是基于强类型语言，对于JavaScript这样灵活的语言，我们并不需要这么麻烦。
